...
function Complementos() {
    // Parámetros no utilizados - se ignoran intencionalmente
    const { 
        sub1: _sub1, sub2: _sub2, sub3: _sub3, sub4: _sub4, sub5: _sub5, 
        tamaño: _tamaño, marca: _marca, tipo: _tipo, estilo: _estilo, id: _id 
    } = useParams();
    ...
    const obtenerRutaExacta = useMemo(() => {
        return () => {
            const path = location.pathname;
            const partes = path.split('/').filter(Boolean);
            const partesRelevantes = partes.slice(1);
    ...
    }, [location.pathname, esPaginaProducto]);
    ...
    useEffect(() => {
        ...
        cargarProductosComplementos();
    }, [location.pathname, esPaginaProducto, obtenerRutaExacta]);
