...
function Dormitorios() {
    ...
    useEffect(() => {
        ...
        const hasOtherFilters = activeFilters.tamaño || activeFilters.marca || 
                               activeFilters.lineaDormitorio || activeFilters.resorte || 
    ...
    // PRIMERO: Aplicar filtros base (tamaño, marca, lineaDormitorio, envio gratis, skus)
    const productosBaseFiltrados = useMemo(() => {
        if (productos.length === 0) return [];
    ...
    // TERCERO: Aplicar filtros jerárquicos (resorte, lineaColchon, modelo, etc.)
    const productosFiltrados = useMemo(() => {
